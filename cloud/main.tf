...
# VPC and Networking using terraform-aws-modules/vpc
module "vpc" {
...
# VPC Endpoint for ECR Docker (ecr.dkr)
...
# VPC Endpoint for S3 (ECR stores image layers in S3)
...
resource "aws_security_group" "ecs_tasks" {
  ...
  ingress {
    ...
    description     = "Allow traffic from ALB to frontend on port 3000"
  }
  ...
  ingress {
    ...
    protocol    = "tcp"
    self        = true
    description = "Allow frontend to communicate with backend internally"
  }
  ...
  egress {
    ...
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
    description = "Allow all outbound traffic for ECR pulls and external APIs"
  }
...
resource "aws_lb_listener_rule" "backend" {
  ...
  tags = {
    Name = "${var.project_name}-backend-routing-rule"
  }
}
...
# Additional policy for ECR access (if using private repositories)
