"""Azure pricing logic — fetches rates from AI or native API, caches in DB."""
...
def _fetch_rate_from_ai(service: str, resource_type: str, region: str) -> dict | None:
    """Ask AI for current Azure pricing and parse the JSON response."""
    if service == "virtual_machines":
        format_hint = '{"hourly_rate_linux": 0.0104, "hourly_rate_windows": 0.0156}'
    elif service == "blob_storage":
        format_hint = '{"per_gb_month": 0.018, "request_per_10000_read": 0.004, "request_per_10000_write": 0.05}'
    elif service == "functions":
        format_hint = '{"per_execution": 0.0000002, "per_gb_second": 0.000016}'
    else:
        format_hint = '{}'

    prompt = f"""Return ONLY a JSON object with Azure pricing. No text, no explanation, no markdown.

Service: Azure {service.replace('_', ' ').title()}
Resource: {resource_type}
Region: {region}

Exact format: {format_hint}

Replace the example values with actual current Azure pricing. Return ONLY the JSON."""
...
def _fetch_rate_from_api(service: str, resource_type: str, region: str) -> dict | None:
    """Fetch pricing from Azure Pricing Calculator API (placeholder/stub).

    TODO: Implement actual Azure Retail Prices API integration.
    Currently returns hardcoded reasonable defaults for supported services.
    """
    stub_rates = {
        "virtual_machines": {"hourly_rate_linux": 0.0104, "hourly_rate_windows": 0.0156},
        "blob_storage": {"per_gb_month": 0.018, "request_per_10000_read": 0.004, "request_per_10000_write": 0.05},
...
def get_rate(db: Session, service: str, resource_type: str, region: str) -> dict | None:
    ...
    rate = db.query(PricingRate).filter(
        PricingRate.service == f"azure_{service}",
...
def get_recommendation(service: str, params: dict, cost: float) -> str:
    """Get a brief AI recommendation for the calculated Azure cost."""
    prompt = f"""You are an Azure cost optimization expert. Be concise — max 3 short sentences.
...
def calculate_virtual_machines(params: dict, db: Session) -> dict:
    ...
    rates = get_rate(db, "virtual_machines", vm_size, region)
    if not rates:
        return {"error": f"Could not fetch pricing for Azure VM {vm_size} in {region}. AI may be unavailable."}

    hourly_rate = _extract_rate(rates, f"hourly_rate_{os_type}", "hourly_rate_linux", "hourly_rate")
    if hourly_rate == 0:
        return {"error": f"Got zero rate for Azure VM {vm_size}. Raw rates: {json.dumps(rates)}"}
...
def calculate_blob_storage(params: dict, db: Session) -> dict:
    storage_gb = params.get("storage_gb", 100)
    region = params.get("region", "eastus")
    access_frequency = params.get("access_frequency", "frequent").lower()

    rates = get_rate(db, "blob_storage", "standard", region)
    if not rates:
        return {"error": f"Could not fetch pricing for Azure Blob Storage in {region}. AI may be unavailable."}
    ...
    estimated_reads = int(storage_gb * 1000 * freq_multiplier.get(access_frequency, 1))
    read_rate_per_10k = _extract_rate(rates, "request_per_10000_read")
    request_cost = round((estimated_reads / 10000) * read_rate_per_10k, 2)
...
def calculate_functions(params: dict, db: Session) -> dict:
    executions = params.get("executions", 1_000_000)
    duration_ms = params.get("duration_ms", 200)
    memory_mb = params.get("memory_mb", 128)
    region = params.get("region", "eastus")

    rates = get_rate(db, "functions", "invocation", region)
    if not rates:
        return {"error": f"Could not fetch pricing for Azure Functions in {region}. AI may be unavailable."}

    per_execution = _extract_rate(rates, "per_execution", "per_request")
    per_gb_second = _extract_rate(rates, "per_gb_second")

    execution_cost = round(executions * per_execution, 4)

    memory_gb = memory_mb / 1024
    duration_sec = duration_ms / 1000
    gb_seconds = executions * memory_gb * duration_sec
    compute_cost = round(gb_seconds * per_gb_second, 4)

    total = round(execution_cost + compute_cost, 2)
...
def calculate_azure_cost(service: str, params: dict, db: Session) -> dict:
    calculators = {
        "virtual_machines": calculate_virtual_machines,
        "blob_storage": calculate_blob_storage,
        "functions": calculate_functions,
    }

    calc = calculators.get(service.lower())
    if not calc:
        return {"error": f"Unsupported service: {service} for azure. Supported: virtual_machines, blob_storage, functions"}
